 ...
 * `wallet-kit.ts`'s `signChallengeMessage`, so a DFNS-only depositor
 * (no StellarWalletsKit connection at all) can still sign the SEP-53
 * correlation read/write challenges (`lib/auth-nonce.ts`). A thin,
 * generic wrapper, matching the pattern already used for deposit and
 * onboarding signing.
 */
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (cause instanceof DfnsRequestError) {
      return Response.json(toErrorEnvelope('MESSAGE_SIGNING_INIT_FAILED', cause.message), {
        status: 502,
      });
    }
    return Response.json(
      toErrorEnvelope('MESSAGE_SIGNING_INIT_FAILED', 'Could not start signing. Try again.'),
      { status: 500 }
    );
  }
}

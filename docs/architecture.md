...
**API Boundaries:**
Corrected 2026-09-21 (Issue #3): the internal API surface is `src/app/api/correlation/[address]/route.ts` (reads gated by the signed-nonce ownership check from `lib/auth-nonce.ts`, Step 4), plus `src/app/api/quote/route.ts` and `src/app/api/deposit/build/route.ts`. 1Click's JWT and DeFindex's API key are real per-partner secrets, verified directly against both SDKs' own documentation, not the public-style tags the original version of this section assumed; `lib/one-click-client.ts` and `lib/defindex-client.ts` are therefore only ever called from these Route Handlers, never imported into client-side code. Horizon carries no credential at all, the only one of the three integration targets actually safe to call directly client-side, exactly as `lib/horizon.ts` already does.

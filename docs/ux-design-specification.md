...
- WCAG AA contrast targets (4.5:1 for body text, 3:1 for large text and UI components) apply to every color pairing in this system, verified by relative-luminance calculation against the dark palette: Signal Cyan, muted text, and the accent-ink button text all clear these targets with comfortable margin. **Re-verified precisely during Story 1.3's implementation (2026-09-21, see `src/lib/contrast.ts`/`contrast.test.ts`):** the base error red (`#E5484D`) clears AA at 4.83:1, not "only narrowly" as this line previously claimed, a real but modest margin, not a near-failure. The underlying design decision stays correct regardless: error and warning **text** specifically still uses the lighter `#FF7A80` variant, which clears AA at 7.52:1, a substantially wider safety margin for exactly the messages the No Silent Failure criterion depends on, so the split between error-ui and error-text colors is justified by the size of the margin difference, not by the base red being borderline.

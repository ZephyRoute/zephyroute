...
- **Integration Track budget-use restriction, same source:** a capped portion of certain tranches may be applied toward validating the product flow with real initial users, but explicitly not toward "broad paid user-acquisition campaigns or incentives tied to on-chain behavior." Permitted spend is described as activities that bring users to the product "to learn what it is, how it works, whether it works, and how it can be improved." Directly relevant to how `scf-interest-form-draft.md`'s Requested Budget Range should eventually be allocated across tranches once a full Build application is drafted, not just the Interest Form stage.

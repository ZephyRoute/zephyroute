...
4. **[PARTIALLY VERIFIED]** What is DeFindex's exact fee schedule, and does the deployed mainnet WASM match the OtterSec-audited, patched commits? Confirmed live: OtterSec audited DeFindex on March 18, 2025 (1 critical, 3 high findings, all resolved per the published report); the "Mainnet Deployment 1.0.0" GitHub release (commit `f2d2785a805f40f5cb05921648ceec345168b7ed`) publishes exact WASM hashes for `blend_strategy` and `defindex_vault` specifically for comparison against Stellar.Expert's deployed contract hash. The final comparison step itself (checking Stellar.Expert's live, client-rendered contract page against these published hashes) was not completed in this pass, since it requires browser automation tooling not loaded in that session; still cheap and non-blocking, still must happen before any "audited" claim is made publicly, per the Risk table.

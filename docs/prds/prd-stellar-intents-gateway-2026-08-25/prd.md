...
7. Is THORWallet's routing already able to add Stellar as a destination, and are they open to embedding this gateway (FR-12) as a corridor? Beyond the business willingness, two technical prerequisites for the iframe-embedded widget approach (`architecture.md`, Starter Template Evaluation) must also be verified: THORWallet's CSP (`frame-ancestors`) must permit framing the Zephyroute embed origin, and that origin must be added to the WalletConnect project's allowed-origins list, or wallet connection fails.

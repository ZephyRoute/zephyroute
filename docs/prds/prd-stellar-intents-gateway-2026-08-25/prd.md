...
- Solana origin routes: 1Click itself now supports Solana USDC (verified live 2026-09-22, Open Question 3), but Solana isn't EVM-compatible, so it needs its own wallet-adapter integration (Phantom/Solflare), not the existing `wagmi` path; a real, separate piece of work, not a gateway blocker anymore. Base is no longer out of scope: added as a fourth live origin route now that it's confirmed working.
...
- **NEAR Intents / 1Click API** (external, production, no SLA held by the gateway), critical path for FR-1 to FR-4. Base origin now live and added as a route; Solana confirmed live at the API level but needs its own non-EVM wallet integration before it can be added (Open Question 3, resolved 2026-09-22).
...
- **Distribution**: THORWallet deprioritized as the primary embed target for FR-12 (decided 2026-09-20, see `addendum.md` §A.4, they already shipped their own native Stellar-plus-Blend loop via NEAR Intents); the widget itself stays generic and the next candidate partner should specifically lack an existing native swap-to-Stellar-yield loop. DeFindex/PaltaLabs remains a candidate revenue-share partner (§7.2, v1.1+ upside, not a dependency).
...
| Risk | Mitigation |
|---|---|
| ~~Base/Solana origin routes stay down beyond a few weeks~~ **[RESOLVED, verified live 2026-09-22]** Base confirmed live and added as a route; Solana confirmed live at the API level, blocked only on its own non-EVM wallet integration, not an upstream outage. Retained for audit history, not an active risk. | Base added to `src/lib/routes.ts`; Solana tracked as its own future wallet-integration scope, not a launch blocker either way. |
...
2. Does settlement strictly require the destination Stellar account to already hold a trustline, or can HOT Bridge fund a brand-new, unfunded account directly? Blocks FR-3 and FR-6's exact shape. **Practical urgency lowered by Open Question 1's resolution (2026-09-20):** on the now-confirmed primary onboarding path (Story 2.2), DFNS establishes the account and trustline during onboarding, before any quote or settlement is requested, so the trustline already exists by the time this question would matter. This question still genuinely requires a real (non-dry) test against a fresh, trustline-less account to answer, so it stays open, not resolved, but it now only gates Story 2.3's manual fallback path and the FR-3 precondition-check's exact phrasing (blocking vs. advisory), not the primary user path.
3. **[BASE RESOLVED, live 2026-09-22, Solana partially resolved]** Re-verified live against 1Click's production API, not assumed stale: `/v0/tokens` now lists real USDC on both `base` (18 tokens total) and `sol` (19 tokens total), and a real `dry: true` quote request to `/v0/quote` for Base USDC to Stellar USDC returned a fully priced, signed quote (correlation ID `bf986d74-2b78-4fe4-8c4e-127680241baf`, `amountOut` ~9.976 USDC for a 10 USDC input), confirming the route works end to end, not just that the token is catalogued; the identical test for Solana USDC to Stellar USDC also succeeded (correlation ID `3255484a-a39c-47c5-9de9-872c03fb39c0`). Base is now added to `epics.md`'s implementation as a fourth live origin route (`src/lib/routes.ts`), since it's EVM-compatible and the existing `wagmi`/`viem` wallet-connection path already covers it with only a chain-config addition. Solana is confirmed working at the API level but not yet added as a route: it isn't EVM-compatible, `wagmi`'s `injected()` connector can't sign for it, and wiring it in needs its own wallet-adapter integration (Phantom/Solflare, a different signing model entirely), a real, separate piece of work tracked here rather than folded into this fix.
...
5. Is the flow legally clear of KYC/AML exposure inherited from 1Click's ToS? If not, what geofencing or screening is required before any SCF submission or public launch claim? **[Confirmatory research only, not a legal resolution, 2026-09-20]** Re-verified live: 1Click's current ToS confirms the compliance obligation sits with the "Developer" integrator, not the end user, and that no user-level KYC is imposed by the API itself; a real precedent (Unstoppable Wallet's own 1Click integration) already operates without requiring user-level KYC. This is useful supporting evidence for the eventual legal review, it is not a substitute for one; the hard gate stays in place, no "KYC-free" claim in any SCF submission or public material until actual legal review confirms it.
...
7. **[RESOLVED for THORWallet specifically, DECIDED 2026-09-20]** Is a partner wallet's routing already able to add Stellar as a destination, and are they open to embedding this gateway (FR-12) as a corridor? THORWallet already added Stellar as a destination in November 2025, using NEAR Intents plus their own native Blend integration, not this gateway (see `addendum.md` §A.4). Decision: THORWallet is deprioritized as the primary Epic 4 target; the widget stays generic and a future candidate partner should specifically lack an existing native swap-to-Stellar-yield loop. The technical prerequisites for the iframe-embedded widget approach (`architecture.md`, Starter Template Evaluation) still apply to whichever partner is pursued: their CSP (`frame-ancestors`) must permit framing the Zephyroute embed origin, and that origin must be added to the WalletConnect project's allowed-origins list, or wallet connection fails.
8. **[CALIBRATION METHOD CONFIRMED, NO TARGET NUMBER YET]** What exact on-chain metric (NAV target vs. cumulative volume, over what window) will the SCF panel ratify for tranche #3 release? Verified live against the Handbook: a well-calibrated commitment must be present and specific (not omitted or vague, a prescreen failure), modest and credible (panels reject both trivially low and unrealistically high thresholds), measured over an agreed window rather than a single-day snapshot, and tied to a registered onchain footprint the panel can check for gaming (self-generated, wash, or sybil activity gets disregarded). No historical numeric NAV/volume thresholds from past Integration Track awards were found in the accessible Handbook, so no specific target is proposed here; this remains genuinely negotiated with the panel, not something to guess at. Directly shapes SM-1/SM-2 targets.
...
10. **[RESOLVED, see `docs/threat-model.md`]** What threat model and monitoring plan will accompany the tranche #2 submission? The SCF Build panel requires both as a tranche #2 condition (verified live against the SCF Handbook). A STRIDE analysis and monitoring plan are now documented, built directly from the failure-mode table in `addendum.md` §C, and found five concrete gaps (CSP `frame-ancestors`/`script-src` hardening, CI secret scanning, error-response sanitization, and access control on the traction-metrics view) already reflected as amendments in `epics.md`.
...
12. **[ADDRESSED at the UX/story level, see `epics.md` Story 1.10]** Does the user's Stellar account, whether pre-existing for UJ-1 or newly created via Privy/DFNS for UJ-2, reliably hold enough XLM to cover the DeFindex deposit transaction's network fee at the moment of the second signature? A pre-signature balance check with a plain-language shortfall message is now specified as an acceptance criterion. Now that Open Question 1 has resolved (DFNS confirmed, with fee-sponsored account creation and trustline setup), whether the account-creation step should also pre-fund a small XLM reserve for this specific deposit-fee purpose, rather than only detecting a shortfall later, is a still-open implementation-detail question, since DFNS's fee sponsorship covers account creation and trustline operations, not the Soroban deposit call itself (confirmed distinct, see Open Question 1's own entry).
13. **[RESOLVED, confirmed by founder 2026-09-20]** Does the team meet the minimum-2-Eligible-Individual requirement the Official Rules state as a hard eligibility condition ("at least 2 Eligible Individuals to be available during the preparation webinars, actively participate in bootcamp activities, and... present during the investor demo day")? Confirmed met. `scf-interest-form-draft.md`'s Team section still needs real names/roles/links filled in before submission, but the eligibility question itself is closed.
14. **[DECIDED, founder 2026-09-20, see `addendum.md` §A.3 for full detail]** Is Integration Track actually the right track for Zephyroute's current pre-code, pre-traction stage, or would Instawards (or waiting until real traction exists) be the more honest fit? The track's own page states it is for teams with an already-live application that already has traction, and explicitly routes net-new, no-traction teams to Instawards instead (corroborated three separate times across Handbook pages). Decision: stay on Integration Track, and treat building genuine, real (non-wash) traction as an explicit project goal rather than a gap to argue around via the "validated need" alternate path alone. This reframes SM-1/SM-2 (§9) and the whole distribution question (Open Question 7) from launch-metrics into a pre-submission priority as well.

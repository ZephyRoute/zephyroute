...
13. **[NEW, found 2026-09-20 reading the Official Rules for Submissions directly]** Does the team meet the minimum-2-Eligible-Individual requirement the Official Rules state as a hard eligibility condition ("at least 2 Eligible Individuals to be available during the preparation webinars, actively participate in bootcamp activities, and... present during the investor demo day")? Not yet checked against this project's actual team composition, since `scf-interest-form-draft.md`'s Team section remains an unfilled placeholder. This is a genuine potential blocker if the team is currently solo, not resolvable by further research, only by the founder's actual team situation.
14. **[NEW, significant strategic question found 2026-09-20 reading the Integration Track page in full, see `addendum.md` §A.3 for full detail]** Is Integration Track actually the right track for Zephyroute's current pre-code, pre-traction stage, or would Instawards (or waiting until real traction exists) be the more honest fit? The track's own page states it is for teams with an already-live application that already has traction, and explicitly routes net-new, no-traction teams to Instawards instead. A real alternate path exists (the Submission Criteria page's "significant user traction, **or** a clearly validated need identified by a team... with relevant experience in the Stellar ecosystem"), but it depends on the actual team's real Stellar/crypto credentials, the same unresolved fact as Open Question 13. Not resolvable by further research; requires the founder's own judgment about the team's real background and risk tolerance, since Instawards themselves are not a substitute path to the same $100K-$150K goal (capped at $15,000, requires local Ambassador Chapter engagement, framed as a precursor rather than an alternative).
